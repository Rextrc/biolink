...
/* Base URL each platform is prefixed with when a bare username is entered
   instead of a full link (e.g. "olik" -> "https://github.com/olik"). A value
   that already looks like a URL (has a scheme) is left untouched. */
...
export function resolveSocialUrl(platform, raw) {
...
/*
 * The olik.app profile card. Purely driven by `cfg` (same shape as
 * DEFAULT_SITE_CFG) so it renders identically for the owner's page and any
 * user page created via /create. Owns only local interaction effects
 * (cursor tilt, live clock, click bursts); data fetching lives in the parent.
 *
 * Props:
 *   cfg        — the profile config object
 *   ready      — gates the card entrance animation
 *   viewCount  — number to show in the watermark, or null to hide
 *   nowPlaying — Spotify now-playing payload, or null (owner page only)
 *   brand      — watermark label, e.g. "olik.app" or "olik.app/jane"
 */
export default function ProfileCard({ cfg, ready = true, viewCount = null, nowPlaying = null, brand }) {
  ...
  const skills = cfg.skills?.length ? cfg.skills : [];
  ...
  return (
    <div style={{
      ...
      {/* Grain — behind the card so backdrop-blur softens it (see CLAUDE.md note) */}
      ...
      <div style={{
        ...
        © {new Date().getFullYear()} {brand || `${(cfg.hero_name || 'olik').toLowerCase()}.app`}
