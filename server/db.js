...
async function init() {
  ...
  try {
    // Admin-created copies of the landing card, each served at olik.app/<slug>.
    // `data` is the same JSON shape as site_config; `edit_code` is the 5-digit
    // code the owner of that page enters at /edit to edit their own content.
    _sql.run(`CREATE TABLE IF NOT EXISTS landing_pages (
      id         INTEGER PRIMARY KEY AUTOINCREMENT,
